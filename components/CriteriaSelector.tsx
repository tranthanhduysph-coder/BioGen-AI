...
// Component Select Field hỗ trợ dịch thuật
...
// Component Queue Item hiển thị tên đã dịch
        ...
        <div className="flex-1 mr-2 overflow-hidden">
            ...
            <div className="text-[10px] text-slate-500 flex flex-wrap gap-1.5">
                ...
                <span className="bg-white dark:bg-slate-800 px-1.5 rounded border border-slate-200 dark:border-slate-600 text-sky-600 font-medium">
                     {/* Chỉ lấy phần tên ngắn của Type */}
...
export const CriteriaSelector: React.FC<CriteriaSelectorProps> = ({ onGenerate, isLoading, onSimulate }) => {
  ...
  const handleChange = (field: keyof Criteria, value: string | number) => {
    setCurrentCriteria(prev => {
        ...
        if (field === 'competency' && String(value).startsWith('nt1')) {
            newData.difficulty = "diff_1"; // Force Recall for NT1
        }
        return newData;
    });
  };
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-none p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
        ...
        {/* Quick Exam */}
        <div className="p-4 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl border border-purple-200 dark:border-purple-700/50 shadow-sm">
            <h3 className="text-sm font-bold text-purple-800 dark:text-purple-200 mb-2 flex items-center gap-2">
             ...
             <div className="mb-3">
                 <textarea
                    ...
                    rows={2}
                    className="w-full bg-white dark:bg-slate-900 border border-purple-200 dark:border-purple-600/50 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-shadow resize-none"
                />
            </div>
            <button onClick={() => onSimulate(simulationPrompt)} disabled={isLoading} className="w-full py-2.5 px-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg shadow-md text-sm flex items-center justify-center transition-transform active:scale-[0.98]">
        ...
        <div className="flex items-center my-4">
            ...
            <span className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wide">{t('criteria.manual_opt')}</span>
            <div className="flex-grow h-px bg-slate-200 dark:bg-slate-700"></div>
        </div>
        ...
        <div className="space-y-4">
            <SelectField label={t('criteria.label_chapter')} value={currentCriteria.chapter} onChange={e => handleChange('chapter', e.target.value)}>
                {/* FIX: Sử dụng t() để dịch Key thành Text hiển thị */}
